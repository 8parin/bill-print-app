    ...
    try {
        ...
        if (data.validation && !data.format_valid) {
            ...
            showNotification(
                ...
                'The uploaded CSV format does not match the expected format.<br>This usually happens when Shopee updates their export format.',
                detailsList
            );
            ...
            showNotification(
                ...
                ['Please re-export the CSV from Shopee and try again.']
            );
            uploadStatus.innerHTML = `⚠️ ${data.message}`;
            uploadStatus.className = 'status-message error';
        } else {
    ...
    try {
        const response = await fetch('/generate', {
            ...
            body: JSON.stringify({
                ...
                starting_bill_number: parseInt(startingBillNumberInput.value) || 2600001
            })
        });
